...
class Canvas
{
  public:
    ...
    {
        canvas = std::vector<std::vector<ColourTuple>>(height, std::vector<ColourTuple>(width, ColourTuple{0, 0, 0}));
    }

    Canvas(uint16_t width, uint16_t height, ColourTuple defaultColour) : width{width}, height{height}
    {
        canvas = std::vector<std::vector<ColourTuple>>(height, std::vector<ColourTuple>(width, defaultColour));
    }
